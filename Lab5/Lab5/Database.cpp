...
Database::Database(DatalogProgram dp)
{
    ...
    graph = Graph(this->rules);

    
    output << "Rule Evaluation\n\n";
    output << graph.dependencyGraphToString();
    output << graph.reverseGraphToString();
    output << graph.postOrderToString();
    output << graph.sccSearchOrder();
    evalGraph();
    //evalRules();
    print();
    evalQueries();
...
void Database::printEvaluation(int i, Relation temp)
{
    ...
    {
        output << "Yes(" << temp.tuples.size() << ")" << endl;
        temp.setName("select");
...
void Database::evalGraph()
{
    vector<int> search_order;
    vector<Rule> temp;
    graph.getSearchOrder(search_order);
    temp = rules;
    ...
    for (unsigned int i=0; i < search_order.size(); i++)
    {
        ...
        if (graph.SCCs[i].size() > 0)
        {
            spacer = true;

            output << "SCC:";
            
            for (auto j : graph.SCCs[search_order[i]])
            {
                output << " R" << j;
...
bool Database::sameName(Rule r)
{
    bool same_name = false;
    
    for (int i=0; i < r.getPredicates().size();i++)
    {
        if (r.getPredicates()[i].name == r.getName().name)
        {
            same_name = true;
        }
    }
...
Relation Database::dbJoin(vector<Relation> relations, Relation temp)
{
    if (relations.size() > 1)
    {
        ...
        {
            Relation r1_temp = relations[k];
            temp = temp.join(r1_temp);
        }
    }
